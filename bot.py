...
# ⚙️ الإعدادات العامة
# ==============================================
# 🚫 ممنوع نهائياً كتابة التوكن كنص افتراضي بالكود (fallback) — لازم من متغيرات البيئة فقط
BOT_TOKEN = os.environ.get("BOT_TOKEN"8792351652:AAET4YWVp2xpOxMOeGgSrqw1MqaniUamtSw")
CHAT_ID_RAW = os.environ.get("CHAT_ID"8674500253")

if not BOT_TOKEN:
    raise RuntimeError("BOT_TOKEN غير موجود في متغيرات البيئة! أضفه في إعدادات Render/Railway ولا تكتبه بالكود أبداً.")
if not CHAT_ID_RAW:
...
CONFIRM_TFS    = ["15د"]
MIN_CONFIRM    = 1
MIN_SCORE      = 6      # تم تخفيفه من 7 → 6
WATCH_SCORE    = 4      # سكور بين 4 و6 = إشارة مراقبة ضعيفة بدل ما تختفي تماماً

ADX_TREND      = 20
ADX_STRONG     = 28

RSI_NEUTRAL_LOW  = 49   # تم تضييقه أكثر من 48-52 إلى 49-51
RSI_NEUTRAL_HIGH = 51
...
# ==============================================
# 📰 الأخبار الاقتصادية (Forex Factory Calendar)
# ==============================================
NEWS_URL = "https://nfs.faireconomy.media/ff_calendar_thisweek.json"
NEWS_CACHE = {"ts": 0, "data": []}
NEWS_TTL = 900  # 15 دقيقة

def fetch_news_calendar():
    """يجلب تقويم الأخبار الاقتصادية لهذا الأسبوع. يفشل بأمان لو المصدر غير متاح."""
    now = time.time()
    ...
    try:
        ...
        return NEWS_CACHE["data"]  # يرجع آخر نسخة محفوظة أو فاضية

def pair_currencies(pair):
    """يستخرج عملات الزوج. الذهب (XAU) بيتأثر أساساً بأخبار الدولار."""
    parts = pair.split("/")
    codes = set()
    for p in parts:
        codes.add("USD" if p == "XAU" else p)
    return codes

def get_news_context(pair):
    """
    يرجع:
    - high_impact_soon: قائمة أخبار عالية التأثير خلال ±45 دقيقة من الآن
    - note: نص جاهز للعرض
    """
    events = fetch_news_calendar()
    codes = pair_currencies(pair)
    ...
    for ev in events:
        try:
            ...
            if country not in codes or impact != "High":
                continue
            ...
            if -45 <= delta_min <= 45:
                title = ev.get("title", "خبر")
                when = "الآن تقريباً" if abs(delta_min) < 5 else (
                    f"خلال {int(delta_min)} د" if delta_min > 0 else f"منذ {int(-delta_min)} د"
                )
                high_soon.append(f"{country} - {title} ({when})")
        except Exception:
            continue

    note = ""
    if high_soon:
        note = "📰 تحذير خبر مؤثر: " + " | ".join(high_soon[:2])
    return {"high_impact_soon": high_soon, "note": note}
...
def CANDLES(df):
    ...
    if body(-2) > 0:
        contained = (max(O[i], C[i]) <= max(O[-2], C[-2])) and (min(O[i], C[i]) >= min(O[-2], C[-2]))
        if contained and body(i) < body(-2) * 0.6:
            if bear(-2) and bull(i):
                res["buy"].append("Harami ↑")
            elif bull(-2) and bear(i):
                res["sell"].append("Harami ↓")

    if bear(-2) and bull(i) and O[i] < L[-2] and C[i] > mid(-2) and C[i] < O[-2]:
        res["buy"].append("Piercing Line ↑")

    if bull(-2) and bear(i) and O[i] > H[-2] and C[i] < mid(-2) and C[i] > O[-2]:
        res["sell"].append("Dark Cloud Cover ↓")

    if len(df) >= 3:
        if bear(-3) and body(-3) > rng(-3)*0.4 and body(-2) <= rng(-2)*0.3 and bull(i) and C[i] > mid(-3):
            res["buy"].append("Morning Star ↑")
        if bull(-3) and body(-3) > rng(-3)*0.4 and body(-2) <= rng(-2)*0.3 and bear(i) and C[i] < mid(-3):
            res["sell"].append("Evening Star ↓")

    return res

# ==============================================
# 🧮 منطق التحليل (الفريم الرئيسي - تحليل كامل)
# ==============================================
def analyze(df, pair=None):
    ...
    if RSI_NEUTRAL_LOW <= rsi <= RSI_NEUTRAL_HIGH:
        logging.info(f"RSI={rsi:.1f} داخل منطقة الحياد الضيقة → لا إشارة")
        return None, 0, {}

    if adx_val >= ADX_STRONG:
        if plus_di.iloc[-1] > minus_di.iloc[-1]:
            buy += 2
            rb.append(f"ADX قوي {adx_val:.0f} +DI مسيطر")
        else:
            sell += 2
            rs.append(f"ADX قوي {adx_val:.0f} -DI مسيطر")
    elif adx_val >= ADX_TREND:
        if plus_di.iloc[-1] > minus_di.iloc[-1]:
            buy += 1
            rb.append(f"ADX معتدل {adx_val:.0f} +DI")
        else:
            sell += 1
            rs.append(f"ADX معتدل {adx_val:.0f} -DI")
    ...
    if rsi < 35:
        ...
        buy += 1
        rb.append(f"RSI {rsi:.0f} ميل هبوطي")
    elif rsi > 55:
        sell += 1
        rs.append(f"RSI {rsi:.0f} ميل صعودي")
    ...
    elif macd.iloc[-1] > sig_macd.iloc[-1]:
        buy += 1
        rb.append("MACD فوق خط الإشارة")
    elif macd.iloc[-1] < sig_macd.iloc[-1]:
        sell += 1
        rs.append("MACD تحت خط الإشارة")
    ...
    weights = {
        ...
        "Harami ↑": 2, "Harami ↓": 2,
        "Piercing Line ↑": 3, "Dark Cloud Cover ↓": 3,
        "Morning Star ↑": 4, "Evening Star ↓": 4,
    }
    ...
    # --- طبقة الأخبار الاقتصادية ---
    news_note = ""
    news_penalty = 0
    if pair:
        ctx = get_news_context(pair)
        if ctx["high_impact_soon"]:
            news_note = ctx["note"]
            news_penalty = 20  # تخفيض ثقة عند وجود خبر مؤثر قريب

    if buy > sell and rsi > 90: return None,0,{}
    if sell > buy and rsi < 10: return None,0,{}

    score = max(buy, sell)
    logging.info(f"تحليل: BUY={buy} | SELL={sell} | SCORE={score} | ADX={adx_val:.1f} | RSI={rsi:.1f}")

    if score < WATCH_SCORE: return None,0,{}
    if buy == sell: return None,0,{}

    tier = "SIGNAL" if score >= MIN_SCORE else "WATCH"

    total = buy + sell
    conf = int((score / total) * 100) if total else 50
    conf = max(30, conf - news_penalty)
    power = min(100, max(20, int(score * 3) - news_penalty))
...
# ==============================================
# 🧭 تأكيد الاتجاه فقط (للفريم الثانوي)
# ==============================================
def confirm_direction(ticker, cfg):
    df = fetch(ticker, cfg["interval"], cfg["period"], cfg["bars"], cfg.get("resample"))
    ...
    ema_dir = "BUY" if e8 > e21 else ("SELL" if e8 < e21 else None)
    macd_dir = "BUY" if macd.iloc[-1] > sig_macd.iloc[-1] else "SELL"

    if ema_dir and ema_dir == macd_dir:
        return ema_dir
    return None
...
def get_multi_timeframe_signal(pair):
    ...
    sig_main, score_main, det_main = analyze(df_main, pair=pair)
    ...
    for tf in CONFIRM_TFS:
        cfg = TIMEFRAMES[tf]
        direction = confirm_direction(ticker, cfg)
        if direction == sig_main:
            confirm_count += 1
    ...
    if confirm_count >= MIN_CONFIRM:
        ...
        det_main["conf"] = min(95, int((new_score / det_main["total"]) * 100) if det_main["total"] else det_main["conf"])
    ...
    det_main["confirm"] = "⚠️ بدون تأكيد من فريم أعلى (ثقة أقل)"
    det_main["conf"] = max(30, det_main["conf"] - 15)
    det_main["power"] = max(20, det_main["power"] - 15)
    return sig_main, score_main, det_main, 0
...
def format_signal(pair, tf, sig, det, exp):
    ...
    tier_label = "⚡ إشارة قوية" if det.get("tier") == "SIGNAL" else "🔎 مراقبة (إشارة ضعيفة نسبياً)"
    news_line = f"\n{det['news']}" if det.get("news") else ""

    return (
        "━━━━━━━━━━━━━━━━━━━━━━━━\n"
        f"⚡ <b>BASILISK v3.5</b> • {otc(pair)}\n"
        f"⏱ {tf} | {exp} | {tier_label}\n{det.get('confirm','')}{news_line}\n"
        "━━━━━━━━━━━━━━━━━━━━━━━━\n"
...
def format_no_signal(pair, tf):
    return f"━━━━━━━━━━━━━━━━━━━━\n⚡ {otc(pair)} | {tf}\n⚠️ السوق متذبذب حالياً (Ranging) - لا يوجد ميل واضح\n━━━━━━━━━━━━━━━━━━━━"
...
def do_scan(chat_id=None, pair=None, tfk=None):
    ...
    if pair and tfk:
        ...
        sig, score, det = analyze(df, pair=pair)
        if sig:
            key = f"{pair}|{tfk}|{sig}"
    ...
    if not found:
        send(tgt, "⚠️ السوق متذبذب على أغلب الأزواج حالياً - لا توجد إشارات حتى ضعيفة", kb_main())
        return
...
def on_command(cid, txt):
    if txt.strip() in ["/start", "/menu"]:
        send(cid,
            "⚡ <b>BASILISK v3.5</b>\n\n"
            "✅ يدوي بالكامل\n"
            "✅ مستوى \"مراقبة\" للإشارات الضعيفة بدل الإخفاء الكامل\n"
            "✅ طبقة أخبار اقتصادية (Forex Factory Calendar) للتحذير من الأخبار المؤثرة\n"
            "✅ ADX + RSI كعوامل تسجيل نقاط متدرجة\n"
            "✅ تأكيد الفريم الثانوي بالاتجاه فقط\n"
            "✅ فريمات أصلية 100%: 1د، 2د، 5د، 15د، 30د، ساعة\n"
            "✅ تكوينات شموع موسّعة: Pin Bar، Engulfing، Doji، 3 Soldiers/Crows،\n"
            "   Harami، Piercing Line، Dark Cloud Cover، Morning/Evening Star\n\n"

import os, time, requests, threading, logging, io
...
def analyze(df):
    ...
    rsi = 100 - 100 / (1 + (cl.diff().clip(lower=0).ewm(14).mean() / (-cl.diff().clip(upper=0).ewm(14).mean() + 1e-9)))
    ...
    if score >= 8:
        return "🟢 BUY", {"price": round(p,5), "score": score, "why": " | ".join(reasons), "stake": round(BALANCE * RISK_PCT / 100, 2)}
    elif score <= -6:
        return "🔴 SELL", {"price": round(p,5), "score": score, "why": " | ".join(reasons), "stake": round(BALANCE * RISK_PCT / 100, 2)}
    return None, {}
...
def telegram_bot():
    ...
    while True:
        try:
            res = requests.get(f"https://api.telegram.org/bot{BOT_TOKEN}/getUpdates?offset={last_update+1}&timeout=10").json()
            for update in res.get("result", []):
                ...
                if "message" in update:
                    ...
                    if text == "/scan" or text == "/start":
                        threading.Thread(target=manual_scan, args=(cid,)).start()
                    elif text == "/help":
...
# ====================== تشغيل ======================
@app.route("/")
def home(): return "✅ BASILISK Manual Scanner جاهز"

...
def analyze(df):
    ...
    if e8>e21>e50: buy +=5; rb.append("EMA صاعد")
    elif e8<e21<e50: sell +=5; rs.append("EMA هابط")
    if rv<30: buy +=4; rb.append("RSI تشبع بيع")
    elif rv>70: sell +=4; rs.append("RSI تشبع شراء")
    if ml.iloc[-2] < ms.iloc[-2] and ml.iloc[-1] > ms.iloc[-1]:
        buy +=4; rb.append("MACD تقاطع صاعد")
    if ml.iloc[-2] > ms.iloc[-2] and ml.iloc[-1] < ms.iloc[-1]:
        sell +=4; rs.append("MACD تقاطع هابط")
    if p <= bl.iloc[-1]: buy +=3; rb.append("دعم بولينجر")
    if p >= bu.iloc[-1]: sell +=3; rs.append("مقاومة بولينجر")
    if sk.iloc[-1]<20: buy +=3; rb.append("ستوكاستيك تشبع بيع")
    if sk.iloc[-1]>80: sell +=3; rs.append("ستوكاستيك تشبع شراء")

    cds = CANDLES(df)
    w = {"Pin Bar ↑":3,"Engulfing ↑":4,"3 Soldiers ↑":5,
         "Pin Bar ↓":3,"Engulfing ↓":4,"3 Crows ↓":5}
    ...
    conf = int((score/(buy+sell))*100) if (buy+sell) else 0
    ...
    det = {
        ...
        "stars":"⭐⭐⭐⭐⭐" if power>=85 else "⭐⭐⭐⭐" if power>=70 else "⭐⭐⭐",
        "grade":"STRONG" if power>=85 else "MEDIUM+" if power>=70 else "MEDIUM",
        "score":score,"conf":conf,"power":power,"bar":"████████████" if power>=85 else "█████████░░░"
    }
...
def get_multi_timeframe_signal(pair):
    ...
    for tf in CONFIRM_TFS:
        ...
        if sig_tf == sig_main and score_tf >= MIN_SCORE - 2:
            confirm +=1

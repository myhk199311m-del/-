...
# ==============================================
# ⚙️ الإعدادات العامة - محسنة لتجنب تقييد الطلبات
# ==============================================
...
# ✅ إعدادات جديدة لتجنب تقييد الطلبات
COOLDOWN       = 600          # انتظار 10 دقائق بين نفس الإشارة
FETCH_DELAY    = 3.5          # انتظار أطول بين كل زوج
MAX_FETCH_RETRIES = 5         # عدد المحاولات عند الفشل
BASE_WAIT      = 2            # وقت الانتظار الأساسي

# أوزان ديناميكية
WEIGHTS = {
    "adx": 3, "ema": 2, "supertrend": 3, "ichimoku": 2,
...
# قائمة أوقات تجنب التداول والأخبار
AVOID_HOURS = [0, 2, 13, 14, 15, 16]
AVOID_DAYS = [5]
NEWS_EVENTS = [
    {"day": 0, "hour": 14, "duration": 2},
    {"day": 2, "hour": 12, "duration": 2},
    {"day": 3, "hour": 13, "duration": 2},
    {"day": 4, "hour": 12, "duration": 2}
]
...
# ==============================================
# 📊 جلب البيانات - معدل لحل مشكلة التقييد
# ==============================================
def fetch(ticker, interval, period, min_bars=60):
    ...
    while retries < MAX_FETCH_RETRIES:
        try:
            ...
            if len(df) < min_bars:
                raise ValueError("عدد الشموع غير كافٍ")
            df["volume"] = 0
            return df.copy()

        except Exception as e:
            err_msg = str(e)
            if "RateLimit" in err_msg or "Too Many Requests" in err_msg or "429" in err_msg:
                wait_time = BASE_WAIT * (2 ** retries)
                logging.warning(f"⚠️ تقييد طلبات لـ {ticker} | محاولة {retries+1}/{MAX_FETCH_RETRIES} بعد {wait_time} ثانية")
                time.sleep(wait_time)
            else:
                logging.warning(f"⚠️ جلب {ticker} فشل: {err_msg}")
                time.sleep(1.5)
            retries += 1

    logging.error(f"❌ تعذر جلب {ticker} بعد {MAX_FETCH_RETRIES} محاولات")
    return pd.DataFrame()
...
def analyze(df):
    ...
    for pat in patterns["buy"]:
        trend_buy += WEIGHTS["candles"]
        reasons.append(pat)
    for pat in patterns["sell"]:
        trend_sell += WEIGHTS["candles"]
        reasons.append(pat)
    ...
    det = {
        ...
        "sl_b": round(price - 1.8*atr_val,5), "tp_b": round(price + 2.7*atr_val,5),
        "sl_s": round(price + 1.8*atr_val,5), "tp_s": round(price - 2.7*atr_val,5),
        "why": " | ".join(reasons)
    }
...
def get_performance_report():
    if not os.path.isfile(LOG_FILE):
        return "📈 لا توجد بيانات سجل حتى الآن"
    try:
        sig_df = pd.read_csv(LOG_FILE)
        total = len(sig_df)
        buy = len(sig_df[sig_df["الإشارة"]=="BUY"])
        sell = len(sig_df[sig_df["الإشارة"]=="SELL"])
        avg_power = round(sig_df["القوة"].mean(),1) if total>0 else 0
        avg_conf = round(sig_df["الثقة"].mean(),1) if total>0 else 0
        return (
            f"📊 <b>تقرير الأداء</b>\n"
            f"━━━━━━━━━━━━━━━━━━━━\n"
            ...
            f"الوضع الحالي: {'مفعل ✅' if TRADING_ENABLED else 'متوقف ⛔'}"
        )
    except:
        return "❌ تعذر قراءة سجل البيانات"

# ==============================================
# 📄 تنسيق الرسائل والفحص
# ==============================================
...
def on_command(cid, txt):
    if txt.strip() in ["/start", "/menu"]:
        send(cid,
            "⚡ <b>BASILISK v4.1 • محسّن وثابت</b>\n\n"
            "✅ حل مشكلة تقييد الطلبات على YFinance\n"
            "✅ معالجة ذكية للأخطاء وإعادة المحاولة\n"
            "✅ إدارة مخاطر متكاملة\n"
            "✅ تحليل متعدد الأطر الزمنية\n"
            "✅ فلتر أخبار وأوقات التداول\n\n"
...
def on_callback(cid, mid, cbid, data):
    ...
    if data == "MAIN":
        ...
        edit(cid, mid,
            ...
            f"ربح متوقع: +${p}\n"
            f"توقف بعد {MAX_CONSECUTIVE_LOSSES} خسائر متتالية\n"
            f"الحد الأقصى للخسارة اليومية: {MAX_DAILY_LOSS}%", kb_main())
...
@app.route("/")
def home():
    return "✅ BASILISK v4.1 • يعمل بنجاح"

if __name__ == "__main__":
    if not BOT_TOKEN or BOT_TOKEN == "ضع_رمز_البوت_هنا":
        print("❌ يجب إدخال رمز البوت BOT_TOKEN أولاً")
    else:

...
# ⚙️ الإعدادات العامة
# ==============================================
# ⚠️ لا تكتب التوكن مباشرة هنا أبداً — استخدم متغيرات البيئة في Render/Railway
...
# --- تم تخفيف بوابة ADX: بدل ما تكون رفض قاطع، بقيت عامل تسجيل نقاط ---
ADX_TREND      = 20   # فوقه = اتجاه واضح (نقاط إضافية)
ADX_STRONG     = 28   # فوقه = اتجاه قوي جداً (نقاط أكبر)

# --- تضييق منطقة حياد RSI بدل استبعادها بالكامل ---
RSI_NEUTRAL_LOW  = 48
RSI_NEUTRAL_HIGH = 52
...
def CANDLES(df):
    """
    يرجع قاموس فيه قوائم أنماط الشراء والبيع المكتشفة على آخر شمعة مغلقة.
    الأنماط: Pin Bar, Engulfing, Doji, Three Soldiers/Crows,
    ...
    # --- Harami (احتواء) ---
    # شمعة كبيرة تليها شمعة صغيرة محتواة داخل جسم الأولى، بلون معاكس = انعكاس محتمل
    if body(-2) > 0:
...
def analyze(df):
    ...
    # منطقة حياد RSI الآن أضيق (48-52) وما بترفض كل شي، بس بتمنع الدخول القريب جداً من 50
    if RSI_NEUTRAL_LOW <= rsi <= RSI_NEUTRAL_HIGH:
        logging.info(f"RSI={rsi:.1f} داخل منطقة الحياد الضيقة → لا إشارة")
        return None, 0, {}

    # ADX أصبح عامل تسجيل نقاط بدل بوابة قاطعة
    if adx_val >= ADX_STRONG:
        ...
        logging.info(f"ADX={adx_val:.1f} < {ADX_TREND} → لا نقاط اتجاه (لكن التحليل مستمر)")
    ...
    if buy == sell: return None,0,{}  # تعادل = لا إشارة واضحة
...
# ==============================================
# 🧭 تأكيد الاتجاه فقط (للفريم الثانوي - أخف بكثير من analyze)
# ==============================================
def confirm_direction(ticker, cfg):
    """
    بدل ما نطبّق كل فلاتر analyze() (ADX/RSI/MIN_SCORE) على فريم التأكيد،
    منكتفي بفحص اتجاه EMA + MACD بس. هاد أهم إصلاح لمشكلة الإشارات المحايدة،
    لأن الفريمين كانوا لازم يعدّوا نفس البوابات القاسية بنفس اللحظة.
    """
...
# ==============================================
# 📊 تحليل متعدد الأطر الزمنية (مُصلَح)
# ==============================================
def get_multi_timeframe_signal(pair):
    ...
    # لم يتأكد الاتجاه من الفريم الثانوي — نرسل الإشارة بثقة أقل بدل رفضها بالكامل
    det_main["confirm"] = "⚠️ بدون تأكيد من فريم أعلى (ثقة أقل)"
    det_main["conf"] = max(50, det_main["conf"] - 15)
    det_main["power"] = max(30, det_main["power"] - 15)
...
def on_command(cid, txt):
    if txt.strip() in ["/start", "/menu"]:
        send(cid,
            "⚡ <b>BASILISK v3.4</b>\n\n"
            "✅ يدوي بالكامل\n"
            "✅ ADX كعامل تسجيل نقاط بدل بوابة قاطعة\n"
            "✅ منطقة حياد RSI مضيّقة (48-52)\n"
            "✅ تأكيد الفريم الثانوي بالاتجاه فقط (أسرع وأدق)\n"

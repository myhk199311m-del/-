...
PAYOUT = 85 # نسبة الربح - عدلها حسب منصتك
...
app = Flask(__name__)
@app.route("/")
def home(): return "✅ BASILISK OTC v5.0 • MANUAL MODE"
...
def analyze_basilisk(df):
    ...
    if adx_val < MIN_ADX:
        return "WAIT", 0, {"reason": "إشارات متضاربة", "adx": adx_val, "rsi": rv, "atr": atr_val}

    if pattern == "M-TOP": sell += 5; rs.append("M-TOP")
    elif pattern == "W-BOTTOM": buy += 5; rb.append("W-BOTTOM")
    ...
    if buy > sell and rv > 85: return "WAIT", 0, {"reason": "إشارات متضاربة", "adx": adx_val, "rsi": rv, "atr": atr_val}
    if sell > buy and rv < 15: return "WAIT", 0, {"reason": "إشارات متضاربة", "adx": adx_val, "rsi": rv, "atr": atr_val}
    ...
    strength = "WEAK+" if conf < 75 else "STRONG"
    stars = "⭐⭐" if conf < 75 else "⭐⭐⭐"
...
# ══ تنسيق الرسالة - نفس الصورة الثانية ═══════════════════════════════
def fmt_basilisk(pair, sig, det):
    ...
    return (
        ...
        f"━━━━━━━━━━━━━━━━━━━━\n"
        f"☑️ EMA {'Bearish' if sig=='SELL' else 'Bullish'} ✓ | MACD - | ADX {'Bear' if sig=='SELL' else 'Bull'} >{MIN_ADX} ✓\n"
        f"☑️ Near {'Resistance' if sig=='SELL' else 'Support'} R1 | {det['candle']} ✓\n"
        f"━━━━━━━━━━━━━━━━━━━━\n"
...
def on_cmd(cid, txt):
    if txt in ["/start", "/menu"]:
        send(cid, "⚡ <b>BASILISK OTC v5.0</b>\n\nاختر زوج للتحليل:", kb_main())

def on_cb(cid, mid, cbid, data):
    answer(cbid)
    if data == "MAIN":
        edit(cid, mid, "⚡ <b>BASILISK OTC v5.0</b>\n\nاختر زوج للتحليل:", kb_main())
    elif data == "PAIRS":

...
# ==============================================
# ⚙️ الإعدادات العامة - الرمز مضاف مباشرة
# ==============================================
BOT_TOKEN = "8553854847:AAF9BKS2ZK8ws7xs11NaZahguFx9i4i4eZ0"
...
# ✅ قائمة أزواج كيوتكس الكاملة
SYMBOLS = {
    "AUD/CAD": "AUDCAD=X", "NZD/CHF": "NZDCHF=X", "USD/EGP": "USDEGP=X",
    "USD/INR": "USDINR=X", "CAD/CHF": "CADCHF=X", "EUR/AUD": "EURAUD=X",
    "GBP/USD": "GBPUSD=X", "NZD/USD": "NZDUSD=X", "USD/CHF": "USDCHF=X",
    "USD/BDT": "USDBDT=X", "USD/PHP": "USDPHP=X", "USD/PKR": "USDPKR=X",
    "EUR/CHF": "EURCHF=X", "EUR/NZD": "EURNZD=X", "NZD/CAD": "NZDCAD=X",
    "USD/NGN": "USDNGN=X", "USD/ZAR": "USDZAR=X", "EUR/USD": "EURUSD=X",
    "USD/ARS": "USDARS=X", "AUD/USD": "AUDUSD=X", "CAD/JPY": "CADJPY=X",
    "USD/COP": "USDCOP=X", "USD/JPY": "USDJPY=X", "CHF/JPY": "CHFJPY=X",
    "EUR/GBP": "EURGBP=X", "GBP/NZD": "GBPNZD=X", "AUD/CHF": "AUDCHF=X",
    "AUD/JPY": "AUDJPY=X", "EUR/CAD": "EURCAD=X", "EUR/JPY": "EURJPY=X",
    "GBP/AUD": "GBPAUD=X", "GBP/JPY": "GBPJPY=X", "NZD/JPY": "NZDJPY=X",
    "USD/CAD": "USDCAD=X", "USD/DZD": "USDDZD=X", "USD/IDR": "USDIDR=X",
    "USD/MXN": "USDMXN=X", "USD/BRL": "USDBRL=X", "GBP/CAD": "GBPCAD=X",
    "GBP/CHF": "GBPCHF=X"
}

# ==============================================
# ⏱ الأطر الزمنية المدعومة أصلاً
# ==============================================
TIMEFRAMES = {
...
def MACD(s):
    ...
    return f - sl, (f - sl).ewm(span=9, adjust=False).mean()
...
def CANDLES(df):
    ...
    if body(i) <= rng(i) * 0.1:
        if C[-2] > O[-2] and C[-3] > O[-3]: res["sell"].append("Doji ↓ (تردد بعد صعود)")
        elif C[-2] < O[-2] and C[-3] < O[-3]: res["buy"].append("Doji ↑ (تردد بعد هبوط)")
    if len(df)>=3:
...
def analyze(df):
    ...
    adx, _, _ = ADX(high, low, close)
    adx_val = float(adx.iloc[-1])
    s2, s1, pivot, r1, r2 = PIVOT(df)
    price = float(close.iloc[-1])
    buy = sell = 0
    rb = rs = []
    if adx_val < ADX_THRESHOLD: return None,0,{}
    if RSI_NEUTRAL_LOW <= rsi <= RSI_NEUTRAL_HIGH: return None,0,{}
    if e8 > e21 and price > e8: buy +=3; rb.append("EMA صاعد + السعر فوق المتوسطات")
    elif e8 < e21 and price < e8: sell +=3; rs.append("EMA هابط + السعر تحت المتوسطات")
    if rsi < 35: buy +=3; rb.append(f"RSI {rsi:.0f} تشبع بيع")
    elif rsi > 65: sell +=3; rs.append(f"RSI {rsi:.0f} تشبع شراء")
    if macd.iloc[-2] < sig_macd.iloc[-2] and macd.iloc[-1] > sig_macd.iloc[-1]: buy +=3; rb.append("MACD تقاطع صاعد")
    if macd.iloc[-2] > sig_macd.iloc[-2] and macd.iloc[-1] < sig_macd.iloc[-1]: sell +=3; rs.append("MACD تقاطع هابط")
    if price <= bb_l.iloc[-1]*1.01: buy +=2; rb.append("دعم بولينجر")
    if price >= bb_u.iloc[-1]*0.99: sell +=2; rs.append("مقاومة بولينجر")
    if stoch_k <25: buy +=2; rb.append("ستوكاستيك منخفض")
    if stoch_k >75: sell +=2; rs.append("ستوكاستيك مرتفع")
    if abs(price-s1) < atr_val*1.2: buy +=2; rb.append(f"قريب من دعم S1={s1}")
    if abs(price-r1) < atr_val*1.2: sell +=2; rs.append(f"قريب من مقاومة R1={r1}")
    closed_df = df.iloc[:-1] if len(df)>5 else df
    patterns = CANDLES(closed_df)
    weights = {"Pin Bar ↑":2,"Engulfing ↑":3,"Pin Bar ↓":2,"Engulfing ↓":3,"Three White Soldiers ↑":3,"Three Black Crows ↓":3}
    for p in patterns["buy"]: buy += weights.get(p,1); rb.append(p)
    for p in patterns["sell"]: sell += weights.get(p,1); rs.append(p)
    ...
    det = {"price":round(price,5),"rsi":round(rsi,1),"adx":round(adx_val,1),"atr":round(atr_val,5),"s1":s1,"r1":r1,"pivot":pivot,"sl_b":round(price-1.5*atr_val,5),"tp_b":round(price+2.5*atr_val,5),"sl_s":round(price+1.5*atr_val,5),"tp_s":round(price-2.5*atr_val,5),"buy":buy,"sell":sell,"total":total,"conf":conf,"power":power,"stake":stake,"profit":profit,"why":" | ".join(rb if buy>sell else rs)}
...
def log_signal(pair,sig,score,power,conf,tf,exp):
    ...
    with open(LOG_FILE,"a",newline="",encoding="utf-8") as f:
        w=csv.writer(f)
        if not exists: w.writerow(["التاريخ","الزوج","الإشارة","النقاط","القوة","الثقة","الإطار","المدة"])
        w.writerow([datetime.now().strftime("%Y-%m-%d %H:%M"),pair,sig,score,power,conf,tf,exp])

def get_report():
    if not os.path.isfile(LOG_FILE): return "📈 لا يوجد سجل إشارات حتى الآن"
    try:
        df=pd.read_csv(LOG_FILE)
        total=len(df); buy=len(df[df["الإشارة"]=="BUY"]); sell=len(df[df["الإشارة"]=="SELL"])
        avg_p=round(df["القوة"].mean(),1); avg_c=round(df["الثقة"].mean(),1)
        return f"📊 <b>تقرير الأداء</b>\n━━━━━━━━━━━━━━━━━━━━\nإجمالي: {total} | شراء: {buy} | بيع: {sell}\nمتوسط القوة: {avg_p}/100 | الثقة: {avg_c}%"
    except: return "📈 سجل فارغ"
...
def format_multi_tf_summary(pair):
    lines=[f"📊 <b>تحليل {otc(pair)}</b>","━━━━━━━━━━━━━━━━━━━━"]
    for key,cfg in TIMEFRAMES.items():
        ...
        if sig:
            arrow="🟢 شراء" if sig=="BUY" else "🔴 بيع"
            lines.append(f"⏱ {cfg['label']}: {arrow} | قوة {det['power']}% | ثقة {det['conf']}%")
        else: lines.append(f"⏱ {cfg['label']}: ⚪ لا إشارة واضحة")
    lines.append("━━━━━━━━━━━━━━━━━━━━\n⚠️ ليس توصية تداول")
    return "\n".join(lines)
...
def do_scan(chat_id=None,pair=None,tfk=None):
    ...
    if pair and tfk:
        ...
        else: send(tgt,format_no_signal(pair,cfg["label"]),kb_result())
        return
    found=[]
    for name in SYMBOLS:
    ...
    if not found: send(tgt,"⚠️ لا توجد إشارات مؤكدة حالياً",kb_main()); return
...
# 🔎 مطابقة اسم الزوج
# ==============================================
def match_pair(user_input):
    cleaned=user_input.strip().upper().replace(" ","")
    for name in SYMBOLS:
        if name.upper()==cleaned or name.upper().replace("/","")==cleaned.replace("/",""): return name
    return None
...
def on_command(cid,txt):
    txt=txt.strip(); parts=txt.split(maxsplit=1); cmd=parts[0] if parts else ""; arg=parts[1] if len(parts)>1 else ""
    if cmd in ["/start","/menu"]:
        send(cid,
            "⚡ <b>BASILISK v3.4</b>\n\n"
            "✅ يدوي بالكامل\n✅ فلتر ADX\n✅ منطقة حياد RSI 45-55\n✅ تحليل متعدد الأطر\n✅ شموع مؤكدة\n\n"
            "📲 أوامر:\n/الكل - فحص كل الأزواج\n/افضل - أزواج مقترحة\n/تحليل EUR/USD - تحليل شامل\n\nأو استخدم القائمة:",kb_main())
    elif cmd=="/الكل": send(cid,"🔍 جاري فحص كل الأزواج..."); threading.Thread(target=do_scan,args=(cid,),daemon=True).start()
    elif cmd=="/افضل": send(cid,"⭐ أزواج مقترحة:\n"+"\n".join(f"• {p} OTC" for p in ["EUR/USD","GBP/USD","USD/JPY","EUR/GBP","USD/CAD"]))
    elif cmd=="/تحليل":
        if not arg: send(cid,"❌ استخدم: /تحليل EUR/USD"); return
        pair=match_pair(arg)
        if not pair: send(cid,"❌ الزوج غير موجود"); return
        send(cid,f"🔍 جاري تحليل {otc(pair)}..."); threading.Thread(target=lambda: send(cid,format_multi_tf_summary(pair)),daemon=True).start()

def on_callback(cid,mid,cbid,data):
    answer(cbid)
    if data=="MAIN": edit(cid,mid,"القائمة الرئيسية",kb_main())
    elif data=="SCANALL": edit(cid,mid,"🔍 جاري الفحص..."); threading.Thread(target=do_scan,args=(cid,),daemon=True).start()
    elif data=="PAIRS": edit(cid,mid,"📊 اختر الزوج:",kb_pairs())
    elif data=="REPORT": edit(cid,mid,get_report(),kb_main())
    elif data=="RISK": s=round(BALANCE*(RISK_PCT/100),2); p=round(s*(PAYOUT/100),2); edit(cid,mid,f"💰 الرصيد: ${BALANCE}\nالمخاطرة {RISK_PCT}% = ${s}\nالربح: +${p}",kb_main())
    elif data.startswith("P:"): edit(cid,mid,f"⏱ اختر الإطار لـ {otc(data[2:])}",kb_tf(data[2:]))
    elif data.startswith("T:"): _,pr,tf=data.split(":"); edit(cid,mid,f"🔍 جاري تحليل {otc(pr)}..."); threading.Thread(target=do_scan,args=(cid,pr,tf),daemon=True).start()
...
def polling():
    ...
    while True:
        try:
            res=requests.get(f"https://api.telegram.org/bot{BOT_TOKEN}/getUpdates",params={"offset":last+1,"timeout":30,"allowed_updates":json.dumps(["message","callback_query"])},timeout=35).json()
            for upd in res.get("result",[]):
                last=upd["update_id"]
                if "message" in upd: on_command(upd["message"]["chat"]["id"],upd["message"].get("text",""))
                if "callback_query" in upd: cb=upd["callback_query"]; on_callback(cb["message"]["chat"]["id"],cb["message"]["message_id"],cb["id"],cb["data"])
        except Exception as e: logging.error(f"Polling: {e}"); time.sleep(3)

@app.route("/")
def home(): return "✅ BASILISK v3.4 • RUNNING"

if __name__ == "__main__":
    threading.Thread(target=polling,daemon=True).start()
    port=int(os.environ.get("PORT",8080))
    try: from waitress import serve; serve(app,host="0.0.0.0",port=port)

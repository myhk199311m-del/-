...
def do_scan(chat_id=None, pair=None, tfk=None):
    """
    pair + tfk  → تحليل زوج واحد بإطار واحد فقط (يدوي)
    pair فقط   → تحليل زوج بكل الأطر التلقائية
    لا شيء     → فحص كل الأزواج تلقائياً
    """
    ...
    is_manual = pair is not None  # ✅ طلب يدوي
    ...
    for name,ticker in pairs.items():
        ...
        for tk,tcfg in tfs.items():
            ...
            # ✅ إذا يدوي: أرسل النتيجة دائماً حتى لو ما في إشارة
            if is_manual and tfk:
                tgt=chat_id if chat_id else CHAT_ID
                if sig:
                    send(tgt,fmt(name,tcfg["label"],sig,det,tcfg["exp"]),kb_result())
                else:
                    send(tgt,
                        f"━━━━━━━━━━━━━━━━━━━━━━━━\n"
                        ...
                        f"⚪ <b>NO SIGNAL</b>\n\n"
                        f"📊 السوق محايد الآن\n"
                        f"<i>جرب إطاراً زمنياً آخر</i>",
                        kb_result())
                return found, best

            if not sig: continue

            # فحص تلقائي — تطبيق الكولداون
            if not is_manual:
                key=f"{name}|{tk}|{sig}"
                with lock:

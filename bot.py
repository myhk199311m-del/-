...
# ⚙️ الإعدادات العامة
# ==============================================
# 🚨 أمان: التوكن والـ CHAT_ID لازم ييجوا من متغيرات البيئة فقط
# لا تكتبي أي قيمة حقيقية هنا كـ default أبداً
BOT_TOKEN = os.environ.get("8792351652:AAFCn7phqBtQPLKY0I2hNKoyRRgsGdrTL8U")
CHAT_ID_RAW = os.environ.get("8674500253")

if not BOT_TOKEN:
    raise SystemExit("❌ لازم تضيفي BOT_TOKEN في environment variables (Render/Railway) قبل التشغيل")
if not CHAT_ID_RAW:
    raise SystemExit("❌ لازم تضيفي CHAT_ID في environment variables قبل التشغيل")
...
# ✅ FIX: ATR بيستخدم دلوقتي Wilder smoothing الصح (ewm بدل rolling mean العادي)
def ATR(h, l, c, p=14):
    tr = pd.concat([h-l, (h-c.shift()).abs(), (l-c.shift()).abs()], axis=1).max(axis=1)
    return tr.ewm(alpha=1/p, adjust=False).mean()

# ✅ FIX: ADX بيستخدم دلوقتي Wilder smoothing الصح على TR وعلى +DM/-DM
def ADX(h, l, c, p=14):
    up = h.diff()
    down = -l.diff()
...
# ✅ FIX: الشموع دلوقتي بتتحلل على آخر شمعة مقفولة فعلياً، مش الشمعة اللي لسه بتتكون
# لما بتستدعي CANDLES لازم تبعتيلها df بدون آخر صف (الشمعة الحية)
def CANDLES(df):
    res = {"buy":[], "sell":[]}
    ...
    i = -1
    if bull(i) and body(i)>0 and (min(C[i],O[i])-L[i]) >= 1.5*body(i): res["buy"].append("Pin Bar ↑")
    if bear(-2) and bull(i) and body(i) > body(-2)*0.8: res["buy"].append("Engulfing ↑")
    if bear(i) and body(i)>0 and (H[i]-max(C[i],O[i])) >= 1.5*body(i): res["sell"].append("Pin Bar ↓")
    if bull(-2) and bear(i) and body(i) > body(-2)*0.8: res["sell"].append("Engulfing ↓")
    return res
...
def analyze(df):
    ...
    # ✅ FIX: نبعت للـ CANDLES نسخة من الداتا من غير آخر شمعة (لسه بتتكون/مش مقفولة)
...
def get_multi_timeframe_signal(pair):
    ...
    if confirm_count >= MIN_CONFIRM:
        # ✅ FIX: بعد ما نضيف نقاط التأكيد، نعيد حساب الثقة (conf) والقوة (power)
        # بدل ما نسيبهم محسوبين على النقاط القديمة قبل التأكيد
...
def do_scan(chat_id=None, pair=None, tfk=None):
    ...
    if pair and tfk:
        ...
        if sig:
            # ✅ FIX: cooldown مطبق دلوقتي حتى على الفحص اليدوي لزوج واحد
            key = f"{pair}|{tfk}|{sig}"

...
def analyze_forced(df):
    """✅ تحليل يدوي — يعطي BUY/SELL حتى لو الإشارة ضعيفة"""
    ...
    if e8>e21>e50:      buy+=4;  rb.append("EMA Bullish")
    elif e8>e21:        buy+=2;  rb.append("EMA Partial Bull")
    elif e8<e21<e50:    sell+=4; rs.append("EMA Bearish")
    elif e8<e21:        sell+=2; rs.append("EMA Partial Bear")

    if rv<30:           buy+=4;  rb.append(f"RSI Oversold {rv:.0f}")
    elif rv<45 and rv>rvp: buy+=2; rb.append(f"RSI Rising {rv:.0f}")
    elif rv>70:         sell+=4; rs.append(f"RSI Overbought {rv:.0f}")
    elif rv>55 and rv<rvp: sell+=2; rs.append(f"RSI Falling {rv:.0f}")

    if mlp<msp and mlv>msv: buy+=4;  rb.append("MACD Cross UP")
    elif mhv>0 and mhv>mhp: buy+=2;  rb.append("MACD UP")
    if mlp>msp and mlv<msv: sell+=4; rs.append("MACD Cross DOWN")
    elif mhv<0 and mhv<mhp: sell+=2; rs.append("MACD DOWN")

    if p<=blv:          buy+=3;  rb.append("BB Lower Band")
    elif p<bmv:         buy+=1;  rb.append("BB Below Mid")
    if p>=buv:          sell+=3; rs.append("BB Upper Band")
    elif p>bmv:         sell+=1; rs.append("BB Above Mid")

    if skv<25 and sdv<25: buy+=2; rb.append(f"Stoch Low {skv:.0f}")
    if skp<sdp and skv>sdv and skv<40: buy+=2; rb.append("Stoch Cross UP")
    if skv>75 and sdv>75: sell+=2; rs.append(f"Stoch High {skv:.0f}")
    if skp>sdp and skv<sdv and skv>60: sell+=2; rs.append("Stoch Cross DOWN")

    if adxv>20:
        if pdiv>ndiv:   buy+=2;  rb.append(f"ADX UP {adxv:.0f}")
        else:           sell+=2; rs.append(f"ADX DOWN {adxv:.0f}")

    if abs(p-s1)<av*1.5: buy+=2;  rb.append(f"Support S1={s1}")
    if abs(p-r1)<av*1.5: sell+=2; rs.append(f"Resistance R1={r1}")
    ...
    # ✅ بدون فلاتر صارمة — نعطي الاتجاه الغالب دائماً
    score=max(buy,sell)
    total=buy+sell
    ...
    if buy>=sell:
        det["cds"]=" | ".join(cds["buy"]) if cds["buy"] else "—"
        det["why"]=" | ".join(rb) if rb else "Mixed signals"
        return "BUY",score,det
    else:
        det["cds"]=" | ".join(cds["sell"]) if cds["sell"] else "—"
        det["why"]=" | ".join(rs) if rs else "Mixed signals"
        return "SELL",score,det
...
def do_scan(chat_id=None, pair=None, tfk=None):
    ...
    if pair and tfk:
        ...
        if sig:
            ...
            # ✅ أرسل تحليلاً تفصيلياً حتى لو NO SIGNAL
